...
# T - test
hist(pop_children)
hist(pop_no_children)
t.test(pop_children, pop_no_children)

# Graph stacked bar charts
plotframe <- data.frame(ability = c("Very Well", "Well", "Not Well", "Not at all"), child = c(prop.table(table(pop_children))), nochild = c(prop.table(table(pop_no_children))))

# Plot bar chart
plot(gvisColumnChart(plotframe, xvar="ability", yvar=c("child", "nochild"), options=list(isStacked=TRUE, title="Parents' Ability to Speak English Stacked By Birth of Child", vAxes="[{title:'Percent of Sub Population'}]", series="[{labelInLegend: 'Child'}, {labelInLegend: 'No child'}]")))
