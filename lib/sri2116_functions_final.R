...
wordcloud(LanguageCorpus, scale=c(4,.5), max.words = 2000, random.order = FALSE, random.color = TRUE, colors = "red", ordered.colors = TRUE)
dev.off()
}

common_languages <- function(population){
  notnative.noteng <- population
  ##Common Languages Spoken at Home by English-Speaking Ability
notnative.noteng %>%
  group_by(LANP) %>%
  summarize(
    Count = n(), na.rm=TRUE) %>%
